<script>
	...
	async function submitReview() {
		...
		const response = await apiService.post('reviews/add', payload);
	...
	async function toggleFavorite() {
		...
		try {
			...
			const response = await apiService.post(endpoint, { productId: id, userId: userId });
...
<div class="font-poppins container mx-auto px-4 py-12">
	{#if product && reviews}
		...
		>
			<div class="overflow-hidden rounded-2xl bg-indigo-100 shadow-xl dark:bg-gray-800">
				<div class="flex flex-col lg:flex-row">
					...
					<div class="flex flex-col justify-between p-6 lg:w-3/5 lg:p-8">
						<div>
							...
							<p
								class="mb-4 text-xl font-semibold text-indigo-600 md:text-2xl dark:text-indigo-400"
							>
								{product.price}
							</p>
							<div class="mb-6 flex flex-wrap gap-2">
								{#each [product.gender, product.category, product.company] as tag}
									<span
										class="rounded-full bg-blue-300 px-3 py-1 text-xs font-medium text-indigo-800 dark:bg-indigo-900 dark:text-indigo-200"
									>
										{tag}
									</span>
								{/each}
							...
							<div class="mb-6 rounded-xl bg-gray-50 p-4 shadow-inner dark:bg-gray-700">
									...
									{#each specifications as spec}
										<div class="flex flex-col">
											<span class="text-sm font-medium text-indigo-600 dark:text-indigo-300"
												>{spec.key}</span
											>
											<span class="text-sm text-gray-800 dark:text-gray-200">{spec.value}</span>
										</div>
									{/each}
								</div>
							</div>
						...
						<div>
							<a
								...
								class="inline-block w-full rounded-lg bg-indigo-600 px-4 py-2 text-center font-bold text-white transition duration-300 ease-in-out hover:bg-indigo-700 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 sm:w-auto dark:focus:ring-offset-gray-800"
							>
		...
		<div class="mt-12">
			<h2 class="mb-6 text-2xl font-bold text-gray-900 dark:text-white">Product Reviews</h2>

			<!-- Sentiment Analysis Chart -->
			<div class="mb-8">
				<h3 class="mb-4 text-xl font-semibold text-gray-900 dark:text-white">Sentiment Analysis</h3>
			...
			<!-- Review Form -->
			<div class="mb-8">
				<h3 class="mb-4 text-xl font-semibold text-gray-900 dark:text-white">Write a Review</h3>
				<textarea
				...
				<button
					...
					class="mt-2 flex items-center justify-center rounded-lg bg-indigo-600 px-4 py-2 font-bold text-white hover:bg-indigo-700 disabled:opacity-50"
				>
			...
			<!-- Review List -->
			<div>
				<h3 class="mb-4 text-xl font-semibold text-gray-900 dark:text-white">Customer Reviews</h3>

				<!-- Sentiment Filter -->
				<div class="mb-4">
					<label for="sentiment-filter" class="mr-2 text-gray-700 dark:text-gray-300"
					...
					<select
						...
						class="rounded border border-gray-300 bg-white px-3 py-2 text-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
					>
				...
				{#each paginatedReviews as review}
					<div class="mb-4 rounded-lg bg-white p-4 shadow dark:bg-gray-800">
						<div class="mb-2 flex items-center">
							...
							{:else}
								<div
									class="mr-2 flex h-10 w-10 items-center justify-center rounded-full bg-gray-200 dark:bg-gray-600"
								>
									<div class="h-6 w-6">
										<FaUser />
									</div>
							...
							<div>
								...
								<p class="text-sm text-gray-600 dark:text-gray-400">{review.created_at.split('T')[0]}</p>
							</div>
						</div>
						<p class="text-gray-800 dark:text-gray-200">{review.user_comment}</p>
						<span
		...
		{#if similarProducts && similarProducts.length > 0}
			<div class="mt-12">
				<h2 class="mb-6 text-2xl font-bold text-gray-900 dark:text-white">Similar Products</h2>
				<div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
					{#each similarProducts as similarProduct}
						<div
							class="hover:scale-102 cursor-pointer overflow-hidden rounded-xl bg-white shadow-md transition-all duration-300 ease-in-out hover:-translate-y-1 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-opacity-50 dark:bg-gray-800"
